...
public class MLPlanARFFExample {
	...
	public static void main(final String[] args) throws Exception {
		...
		/* open visualization */
		new JFXPanel();
		GraphVisualizationWindow window = new GraphVisualizationWindow(mlplan, new GraphViewPlugin(), new NodeInfoGUIPlugin<>(new JaicoreNodeInfoGenerator<>(new TFDNodeInfoGenerator())),
				new NodeInfoGUIPlugin<>(new TFDNodeAsCIViewInfoGenerator(mlplan.getComponents())), new SearchRolloutHistogramPlugin<>(), new SolutionPerformanceTimelinePlugin(), new OutOfSampleErrorPlotPlugin(split.get(0), split.get(1)));
